...
REM Generate version tag with timestamp
for /f "tokens=2 delims==" %%I in ('wmic os get localdatetime /value') do set datetime=%%I
set VERSION_TAG=v5-%datetime:~0,8%-%datetime:~8,6%
...
docker build -f Dockerfile.transcribe -t %IMAGE_NAME%:%VERSION_TAG% --build-arg WHISPER_MODEL=%WHISPER_MODEL% .
if errorlevel 1 (
...
docker tag %IMAGE_NAME%:%VERSION_TAG% %ECR_REPO%:%VERSION_TAG%
if errorlevel 1 (
    echo ERROR: Failed to tag image with version
    exit /b 1
)
docker tag %IMAGE_NAME%:%VERSION_TAG% %ECR_REPO%:%LATEST_TAG%
if errorlevel 1 (
...
if errorlevel 1 (
    echo ERROR: Failed to push versioned image to ECR
    exit /b 1
)
echo Pushing latest tag...
docker push %ECR_REPO%:%LATEST_TAG%
if errorlevel 1 (
    echo ERROR: Failed to push latest image to ECR
    exit /b 1
)
echo.
...
echo.
echo ECR Image URIs:
echo   Version: %ECR_REPO%:%VERSION_TAG%
...
echo 4. Use image URI (choose one):
echo    - For this specific version: %ECR_REPO%:%VERSION_TAG%
echo    - For latest version:       %ECR_REPO%:latest
